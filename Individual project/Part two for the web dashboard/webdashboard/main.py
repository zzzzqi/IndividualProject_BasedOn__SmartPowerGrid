...
## Date: 11 Oct 2022
## By: Individual project - based on Group F Toumetis - MSc CS Final Project - University of Bristol
...
hv.extension('bokeh')
pn.extension('vega', 'tabulator',nthreads=20)
...
# Ensure that all sliders only update when mouse released,
# so that it can improve the fluency of web page and the user experience.
...
def dynamic_env(read_file):
    ...
    if read_file is None:
        ...
        if 'metadata_df' not in pn.state.cache and 'df' not in pn.state.cache:
            pn.state.cache['metadata_df'] = metadata_df = pd.read_csv(io.BytesIO(file_input.value), header=0,
                                                                      usecols=range(0, 4))
            pn.state.cache['df'] = df = pd.read_csv(io.BytesIO(file_input.value), header=0, usecols=range(4, 64))
        else:
            metadata_df = pn.state.cache['metadata_df']
    ...
    # the event selection on the data-exploraton pane when selecting an event.
    def build_event_page_single_selection(
            selection,
            selected_df,
            x_axis,
            y_axis,
        ...
        event_waveform_predictons.append(pn.Spacer(
            background="lightgrey",
            width=1,
            height=110
        )
        )
        event_waveform_predictons.append(pn.Column(
            f"""
    ...
    # Build the dynamic event page
    # Dynamic in the sense that it depends on
    # the event selection on the data-exploraton pane when selecting multiple events.
    def build_event_page_multiple_selection(
            selection,
            selected_df,
            x_axis,
            y_axis,
        ...
        # built a Tabulator object that contains multiple selected events.
        query = ' & '.join(
            f'{crange[0]:.3f} <= `{col}` <= {crange[1]:.3f}'
            for col, crange in selection.items()
        )
        multiple_selection_df = pd.DataFrame(selected_df.query(query))
        multiple_selection_events_tabulator = pn.widgets.Tabulator(
            multiple_selection_df,
            header_filters=True,
        ...
        # Build a Button to display the selected events
        display_selected_events_button = pn.widgets.Button(
            name="Display selected events from table below",
            button_type="success",
            width=200,
            margin=(0, 0, 0, 0)
        )
        ...
        def _download_callback():
            ...
            """
            sio = io.StringIO()
            events_selection = multiple_selection_events_tabulator.selection
            events_df_copy = multiple_selection_df.reset_index()

            selected_events_df = \
                pd.DataFrame(columns=events_df_copy.columns)
            for i in range(len(events_selection)):
                target_index = events_selection[i]
                selected_events_df.loc[i] = \
                    (events_df_copy.iloc[target_index])

            selected_events_df.insert(
                len(selected_events_df.columns),
                'group_name',
                group_name_widgets.value)

            selected_events_df.to_csv(sio, index=False)
            sio.seek(0)
            return sio

        thread = threading.Thread(target=_download_callback)
        thread.start()
        ...
        # Build the entire event page with event data and waveforms
        first_layout_layer1 = pn.Row(
                        pn.Column(
                            f"""
                                        ** _Identifying events:_ **
                                        <ul>
                                        <li> A total of {str(len(multiple_selection_df.index))}  events are selected from data-exploration pane.
                                        <li> To display the details of selected events from the table below:
                                        </ul>
        ...
        first_layout_layer1.append(pn.Column(
                            "<br>",
                            display_selected_events_button,
                        )
        )
        first_layout_layer2 = pn.Row(
                        pn.Column(
                            """
                        ** _Labelling a group of events:_ **
                        <ul>
                        <li> To enter the group name for the selected events from the table below: <br>
        ...
        second_widgetBox = pn.WidgetBox(
                    "#### Selected events from data-exploration pane:",
                    multiple_selection_events_tabulator,
                    pn.Spacer(
                        background="lightgrey",
                        width=975,
                        height=1
        ...
        second_layout_layer = pn.Column(
            first_widgetBox
        )
        second_layout_layer.append(
            second_widgetBox
        )
        event_page = pn.Row(
            second_layout_layer
        ...
        # Build a dictionary to cache the loaded CSV data of events
        events_dict = dict()

        # Build the dynamic event page for the selected events
        # Dynamic in the sense that it depends on
        # the selected events by the user on the Tabulator object
        # and the clicking of the "display_selected_events_button"
        @pn.depends(display_selected_events_button)
        def build_events(_):
            """
            This method returns the dynamic event page(s) for the selected
            event(s).
            It depends on the events selected by the user on the
            Tabulator object, and the clicking of the display button.

            :return: the dynamic event page(s) that show(s) the event data, and
                the event waveforms of the selected event(s)
            """
            events_selection = multiple_selection_events_tabulator.selection

            if bool(events_selection):
                events_df_copy = multiple_selection_df.reset_index()

                events_row = pn.Row()
                events_row.append(pn.Spacer(
                    background="lightgrey",
                    width=1,
                    height=1260
                )
                ...
                for i in range(len(events_selection)):
                    target_index = events_selection[i]
                    target_event_df = events_df_copy.iloc[target_index]
                    target_event_id = events_df_copy.iloc[
                        target_index,
                        events_df_copy.columns.get_loc("event_id")
                    ]

                    # Event page element - header
                    target_event_header = "#### selected event number " + str(i + 1) + ":"
                    # Read the event from the cache dictionary if it is there
                    # Otherwise load its CSV file
                    if target_event_id in events_dict:
                        events_row.append(pn.Spacer(
                            background="lightgrey",
                            width=1,
                            height=1260
                        ...
                        events_row.append(pn.Column(
                            target_event_header,
                            events_dict.get(target_event_id)[0],
                            events_dict.get(target_event_id)[1]
                        )
                        )
                    else:
                        # Read the target event CSV file
                        target_event_csv_filename = \
                            os.getcwd() + os.sep + "event_data" + os.sep + \
                            target_event_df["input_event_csv_filename"] + ".csv"
                        target_event_details_df = \
                            pd.read_csv(target_event_csv_filename, header=0)

                        # Extract the event data
                        target_event_start_time = \
                            target_event_details_df["start_time"].values[0]
                        target_event_asset_name = \
                            target_event_details_df["asset_name"].values[0]
                        target_event_x_value = target_event_df[x_axis]
                        target_event_y_value = target_event_df[y_axis]
                        target_event_predictions_dict = \
                        ...
                        target_event_data = pn.Row(
                            pn.Column(
                                f"""
                                        ** _Event metadata:_ **
                                        <ul>
                                        <li> event_id: {str(target_event_id)}
                                        <li> start_time: {str(target_event_start_time)}
                                        <li> asset_name: {str(target_event_asset_name)}
                                        """,
                                width=300
                            )
                        )
                        ...
                        target_event_data.append(
                            pn.Column(
                                f"""
                                        ** _Axis values:_ **
                                        <ul>
                                        <li> {str(x_axis)}: {"%.5f" % target_event_x_value}
                                        <li> {str(y_axis)}: {"%.5f" % target_event_y_value}
                                        """,
                                width=300
                            )
                        )
                            ...
                            target_event_data.append(
                                pn.Column(
                                    f"""
                                            ** _Cluster value:_ **
                                            <ul>
                                            <li> cluster: {str(target_event_df["cluster"])}
                                            """,
                                )
                            )
                        target_event_waveform_predictons = pn.Row(
                            pn.Column(
                                f"""
                                        ** _Top PQD predictions for event's voltages:_ **
                                        <ul>
                                        <li> Vab: {target_event_predictions_dict["Vab"][0]} 
                                            ({"%.5f" % target_event_predictions_dict["Vab"][1]})
                                        <li> Vbc: {target_event_predictions_dict["Vbc"][0]} 
                                            ({"%.5f" % target_event_predictions_dict["Vbc"][1]})
                                        <li> Vca: {target_event_predictions_dict["Vca"][0]} 
                                            ({"%.5f" % target_event_predictions_dict["Vca"][1]})
                                        </ul>
                                        """,
                                width=300
                            )
                        )
                        target_event_waveform_predictons.append(
                            pn.Spacer(
                                background="lightgrey",
                                width=1,
                                height=110
                        ...
                        target_event_waveform_predictons.append(
                            pn.Column(
                                f"""
                                        ** _Top PQD predictions for event's currents:_ **
                                        <ul>
                                        <li> Ia: {target_event_predictions_dict["Ia"][0]} 
                                            ({"%.5f" % target_event_predictions_dict["Ia"][1]})
                                        <li> Ib: {target_event_predictions_dict["Ib"][0]} 
                                            ({"%.5f" % target_event_predictions_dict["Ib"][1]})
                                        <li> Ic: {target_event_predictions_dict["Ic"][0]} 
                                            ({"%.5f" % target_event_predictions_dict["Ic"][1]})
                                        </ul>
                                        """,
                                width=300
                            )
                        ...
                        voltages_waveforms = target_event_details_df.hvplot.line(
                            x='timestamps (in ms)',
                            y=['Vab', 'Vbc', 'Vca'],
                            legend=True,
                            responsive=False,
                        ...
                        currents_waveforms = target_event_details_df.hvplot.line(
                            x='timestamps (in ms)',
                            y=['Ia', 'Ib', 'Ic'],
                            legend=True,
                            responsive=False,
                        ...
                        target_event_data_box = pn.WidgetBox(
                            "#### Event data:",
                            target_event_data,
                            pn.Spacer(
                                background="lightgrey",
                            ...
                            target_event_waveform_predictons,
                            width=1000
                        )
                        # Event page element - waveforms
                        target_event_waveforms_box = pn.WidgetBox(
                            "#### Event waveforms:",
                            voltages_waveforms,
                            currents_waveforms,
                            width=1000
                        ...
                        events_row.append(pn.Column(
                            target_event_header,
                            target_event_data_box,
                            target_event_waveforms_box
                        )
                        )
                        # Store the elements in the cache dictionary
                        events_dict[target_event_id] = \
                            [target_event_data_box, target_event_waveforms_box]
                return events_row


        event_page.append(build_events)
        return event_page
    ...
    def build_similar_event_page_single_selection(
            selection,
            selected_df,
            x_axis,
            y_axis,
        ...
        t = time.perf_counter()
        # Extract the event data
        # print(" type of selection is :")
        # print(type(selection))
        # print(selection)
        # print("selected_df is :")
        # print(type(selected_df))
        # print(selected_df)
        selected_event_df = selected_df.iloc[selection[0] - 1]
        # print("type of selected_event_df is :")
        # print(type(selected_event_df))
        # print(selected_event_df)
        selected_event_x_value = selected_event_df[x_axis]
        selected_event_y_value = selected_event_df[y_axis]
        ...
        # Build an entire similar events page
        similar_events_page_first_layer1 = pn.Row(
                    pn.Column(
                        f"""
        ...
        similar_events_page_first_layer2 = pn.Spacer(
                    width=950,
                    height=1,
                    background="lightgrey"
                )
        similar_events_page_first_layer3 = pn.Row(
                    pn.Column(
                        """
                        ** _Labelling a group of events:_ **
                        <ul>
        ...
        similar_events_page_first_layer4 = pn.Spacer(
                    width=950,
                    height=1,
                    background="lightgrey"
                )
        similar_events_page_first_layer5 = similar_events_tabs

        similar_events_page_second_layer = pn.Column()
        similar_events_page_second_layer.append(similar_events_page_first_layer1)
        similar_events_page_second_layer.append(similar_events_page_first_layer2)
        similar_events_page_second_layer.append(similar_events_page_first_layer3)
        similar_events_page_second_layer.append(similar_events_page_first_layer4)
        similar_events_page_second_layer.append(similar_events_page_first_layer5)
        ...
        similar_event_page = pn.Row(
            pn.Column(
                ...
                "#### Similar events:",
                pn.WidgetBox(
                    similar_events_page_second_layer,
                    width=1000
                )
            )
        )
        ...
        def build_similar_events(_):
            ...
            if bool(similar_events_selection):
                ...
                similar_events_row.append(build_event_page_single_selection(
                    selection,
                    selected_df,
                    x_axis,
                    y_axis,
                ...
                for i in range(len(similar_events_selection)):
                    ...
                    if target_similar_event_id in similar_events_dict:
                        ...
                        target_similar_event_waveform_predictons.append(
                            pn.Spacer(
                                background="lightgrey",
                                width=1,
                                height=110
        ...
        print(f'After optimization, constructing similar events tabs consume : {time.perf_counter() - t:.8f} s')
        # Return the similar event page
        return similar_event_page

    # Build the dynamic similar event page
    # Dynamic in the sense that it depends on
    # the multiple events selection on the data-exploraton pane,
    # and the similar events parameters selected
    def build_similar_event_page_multiple_selection(
            selection,
            selected_df,
            x_axis,
            y_axis,
        ...
        multiple_selection_df = pd.DataFrame(selected_df.query(query))
        # print("multiple_selection_df :")
        # print(multiple_selection_df)

        if len(multiple_selection_df) != 0:
            selected_event_df = multiple_selection_df.iloc[0]
            selected_event_x_value = selected_event_df[x_axis]
            selected_event_y_value = selected_event_df[y_axis]
            ...
            for x in range(len(multiple_selection_df) - 1):
                selected_event_df = multiple_selection_df.iloc[x + 1]
                selected_event_x_value = selected_event_df[x_axis]
                selected_event_y_value = selected_event_df[y_axis]
                # print("selected_event_df :")
                # print(selected_event_df)
                # print(selected_event_x_value)
                # print(selected_event_y_value)
                ...
                if (clusters == None):
                    # For no Dimensionality Reduction algos
                    if (x_axis != "PC 1" and x_axis != "PC 2" and x_axis != "UMAP 1" and x_axis != "UMAP 2"):
                        similar_events_df_append = selected_df.loc[(
                                (selected_df[x_axis] <= selected_event_x_value + similar_events_x_parameter) &
                                (selected_df[x_axis] >= selected_event_x_value - similar_events_x_parameter) &
                                (selected_df[y_axis] <= selected_event_y_value + similar_events_y_parameter) &
                                (selected_df[y_axis] >= selected_event_y_value - similar_events_y_parameter))]
                ...
                # For clustering algos
                    ...
                    # Add cluster value to the similar events dataframe
                    selected_event_cluster = selected_event_df["cluster"]
                    similar_events_df_append = selected_df.loc[(selected_df["cluster"] == selected_event_cluster)]
                    # Calculate the Euclidean distance of the similar events in the same cluster to the selected event
                    # Then add it to the similar events dataframe
                    euclidean_distances = []
                    for i in range(len(similar_events_df_append)):
                        x_distance = similar_events_df_append.iloc[i][x_axis] - selected_event_x_value
                        y_distance = similar_events_df_append.iloc[i][y_axis] - selected_event_y_value
                        euclidean_distances.append(sqrt(x_distance ** 2 + y_distance ** 2))
                    similar_events_df_append.insert(
                        len(similar_events_df_append.columns) - 1,
                        "Euclidean distance",
                        euclidean_distances
                    )
                similar_events_df_append.set_index("event_id", inplace=True)
                pd.concat([similar_events_df, similar_events_df_append])
            ...
            # Build an entire similar events page
            similar_event_page_first_layer1 = pn.Row(
                        pn.Column(
                            f"""
                            ** _Identifying similar events:_ **
                            <ul>
            ...
            similar_event_page_first_layer2 = pn.Spacer(
                        width=950,
                        height=1,
                        background="lightgrey"
                    )
            similar_event_page_first_layer3 = pn.Row(
                        pn.Column(
                            """
                            ** _Labelling a group of events:_ **
                            <ul>
            ...
            similar_event_page_first_layer4 = pn.Spacer(
                        width=950,
                        height=1,
                        background="lightgrey"
                    )
            similar_event_page_first_layer5 = similar_events_tabs
            similar_event_page_second_layer = pn.Column()
            similar_event_page_second_layer.append(similar_event_page_first_layer1)
            similar_event_page_second_layer.append(similar_event_page_first_layer2)
            similar_event_page_second_layer.append(similar_event_page_first_layer3)
            similar_event_page_second_layer.append(similar_event_page_first_layer4)
            similar_event_page_second_layer.append(similar_event_page_first_layer5)
            ...
            similar_event_page = pn.Row(
                pn.Column(
                    ...
                    "#### Similar events:",
                    pn.WidgetBox(
                        similar_event_page_second_layer,
                        width=1000
                    )
                )
            )
    ...
    def data_exploration(dr_value, clustering_value,
        ...
        check_multiple_selection = pn.widgets.RadioButtonGroup(
            name='Radio Button Group', options=['Single events selection', 'multiple events selection'],
            button_type='default', width=600)
        ...
        if dr_value == "Nil" and clustering_value == "Nil":
            ...
            plot_single_selection = alt.Chart(selected_df).mark_circle(size=80).encode(
            ...
            vega_pane_single_selection = pn.pane.Vega(plot_single_selection, debounce=10)

            brush = alt.selection_interval(name='brush')
            plot_multiple_selction = alt.Chart(selected_df).mark_circle(size=80).encode(
                x=alt.X(basic_x_value, scale=alt.Scale(domain=[-0.1, 1.1])),
                y=alt.Y(basic_y_value, scale=alt.Scale(domain=[-0.1, 1.1])),
                color=alt.condition(brush, alt.value("navy"), alt.value('lightgray')),
                tooltip=["event_id", basic_x_value, basic_y_value]
            ).properties(
                height=data_exploration_pane_height,
                width=data_exploration_pane_width
            ).add_selection(brush)
            vega_pane_multiple_selection = pn.pane.Vega(plot_multiple_selction, debounce=10)

            # Build the interactive event tab when selecting an event.
            def get_event_single_selection(selection):
                if not selection:
                    return '### No event selection'
                else:
                    return build_event_page_single_selection(
                        selection,
                        selected_df,
                        basic_x_value,
                        basic_y_value,
            ...
            # Build the interactive event tab when selecting multiple events.
            def get_event_multiple_selection(selection):
                if not selection:
                    return '### No event selection'
                else:
                    return build_event_page_multiple_selection(
                        selection,
                        selected_df,
                        basic_x_value,
                        basic_y_value,
            ...
            # Build the interactive similar events tab when selecting an event.
            def get_similar_events_single_selection(selection):
                if not selection:
                    return '### No event selection'
                else:
                    # print("vega_pane_single_selection.selection.param.event_id////")
                    # print(type(vega_pane_single_selection.selection.param.event_id))
                    # print(vega_pane_single_selection.selection.param.event_id)
                    return build_similar_event_page_single_selection(
                        selection,
                        selected_df,
                        basic_x_value,
                        basic_y_value,
            ...
            # Build the interactive similar events tab when selecting multiple events.
            def get_similar_events_multiple_selection(selection):
                if not selection:
                    return '### No event selection'
                else:
                    return build_similar_event_page_multiple_selection(
                        selection,
                        selected_df,
                        basic_x_value,
                        basic_y_value,
            ...
            @pn.depends(check_multiple_selection)
            def display_data_exploration_pane(check_multiple_selection_value):
                if check_multiple_selection_value == "multiple events selection":
                    return vega_pane_multiple_selection
                else:
                    return vega_pane_single_selection

            @pn.depends(check_multiple_selection)
            def display_selected_events(check_multiple_selection_value):
                if check_multiple_selection_value == "multiple events selection":
                    return pn.bind(get_event_multiple_selection, vega_pane_multiple_selection.selection.param.brush)
                else:
                    return pn.bind(get_event_single_selection, vega_pane_single_selection.selection.param.event_id)

            @pn.depends(check_multiple_selection)
            def display_similar_events(check_multiple_selection_value):
                if check_multiple_selection_value == "multiple events selection":
                    return pn.bind(get_similar_events_multiple_selection,
                                   vega_pane_multiple_selection.selection.param.brush)
                else:
                    return pn.bind(get_similar_events_single_selection,
                                   vega_pane_single_selection.selection.param.event_id)

            return pn.Tabs(
                ("Data-exploration pane", panel.Column(check_multiple_selection, display_data_exploration_pane)),
                ("Selected event", display_selected_events),
                ("Similar events", display_similar_events)
            )
        ...
        # Option B: PCA and no clustering algo
            ...
            brush = alt.selection_interval(name='brush')
            plot_multiple_selection = alt.Chart(selected_df).mark_circle(size=80).encode(
                x=pca_x_value,
                y=pca_y_value,
                color=alt.condition(brush, alt.value("navy"), alt.value('lightgray')),
                tooltip=["event_id", pca_x_value, pca_y_value]
            ...
            vega_pane_multiple_selection = pn.pane.Vega(plot_multiple_selection, debounce=10)
